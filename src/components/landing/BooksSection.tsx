...
export function BooksSection() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/books`)
      .then((r) => r.json())
      .then((json) => { if (json?.success && Array.isArray(json.data)) setBooks(json.data); })
      .catch(() => {})
        ...
        <div className="mx-auto grid max-w-[960px] grid-cols-2 gap-5 md:grid-cols-4">
          {books.map((book) => (
            <div key={book.id}
              ...
              <div className="h-[3px]" style={{ background: book.highlight ? "linear-gradient(90deg, #C93040, #A31D2B)" : "linear-gradient(90deg, #C9A84C, #E8D48B)" }} />
