// FILE: src/app/bai-tap-tuong-tac/[id]/page.tsx — Khách làm bài PUBLIC (gap/MC/matching, không lưu điểm)
"use client";
...
export default function PublicDoExercisePage() {
  ...
  const unitLabel = isMatching ? "cặp nối" : (exercise.type === "MULTIPLE_CHOICE" || exercise.type === "QUIZ" || exercise.type === "VOCAB_CHECK") ? "câu hỏi" : "chỗ trống";
    ...
    <>
      ...
      <main className="mx-auto max-w-[760px] px-6 py-12">
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-xs text-muted">{totalCount} {unitLabel} · Làm thử miễn phí (không lưu điểm)</p>
        </div>
        ...
        {result && (
          <div className="mb-6 rounded-2xl border border-gold/30 bg-gradient-to-br from-gold/10 to-amber-50 p-6 text-center">
            ...
            <p className="text-sm text-muted">Đúng {result.correct}/{result.total}</p>
        ...
        {isGapEx ? (
          <div className="rounded-xl border border-silver/20 bg-white p-5">
            <GapPlayer
        ...
        {!result && (
          <button onClick={handleSubmit} disabled={submitting || !allAnswered}
            ...
            {submitting ? <><Loader2 size={16} className="animate-spin" />Đang chấm...</> : <><Send size={16} />Nộp bài ({answeredCount}/{totalCount})</>}
          </button>
        )}
