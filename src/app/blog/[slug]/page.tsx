 ...
 * MÔ TẢ: Chi tiết bài viết (public) — đọc từ Zustand store + bài viết liên quan
 *         Khi có Backend → chuyển thành Server Component + fetch API + generateMetadata
 */
...
export default function BlogPostPage() {
  ...
  const relatedPosts = allPosts.filter((p) => p.status === "PUBLISHED" && p.slug !== slug).slice(0, 3);
      ...
      <main>
        ...
        {post.thumbnailUrl && (
          <div className="mx-auto -mt-8 max-w-[840px] px-6">
            <img src={post.thumbnailUrl} alt={post.title} className="w-full rounded-2xl object-cover shadow-lg" style={{ aspectRatio: "16/9" }} />
          </div>
        )}
              ...
              {relatedPosts.map((r) => (
                <Link key={r.id} href={`/blog/${r.slug}`}
                  ...
                  <div className="aspect-[16/10] overflow-hidden">
                    <img src={r.thumbnailUrl} alt={r.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    ...
                    <p className="mt-2 text-xs text-muted">{r.readTime} · {r.createdAt}</p>
                  </div>
                </Link>
              ))}
            </div>
